...
namespace bike.Controls.SkColorDispersionMap
{
    public partial class SessionMap
    {
        ...
        {
            const string StartImageName = ".Assets.stopwatch-solid.svg";
            const string EndImageName = ".Assets.flag-checkered-solid.svg";
            using (var stream = GetType().GetTypeInfo().Assembly.GetManifestResourceStream(StartImageName))
            {
                _startImage = new SKSvg();
                _startImage.Load(stream);
            }

            using (var stream = GetType().GetTypeInfo().Assembly.GetManifestResourceStream(EndImageName))
            {
                _endImage = new SKSvg();
        ...
        private void MapOnPaintSurface(object sender, SKPaintSurfaceEventArgs e)
        {
            ...
            _positionConverter.UpdateCamera(Layer.GeoCoordinates.ToLatLong(),Map.ZoomLevel, new Size(info.Width, info.Height), SkiaHelper.PixelPerUnit);
